...
async def profile_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        my_message = await send_text(update, context, "ChatGPT занимается генерацией вашего профиля, подождите несколько секунд...")
...
async def opener_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        dialog.user["age"] = text
        await send_text(update, context, "Оцените ее внешность: 1-10 балов")
    elif dialog.count == 3:
...
chatgpt = ChatGptService(token="token gpt")
app = ApplicationBuilder().token("chat's token").build()
